...
overall = [a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t]

def compute_list(lst):
    lsum = 1
    for x in lst:
        lsum *= int(x)
    return lsum
...
for arr in overall:
    for i in xrange(0, 17):
        ...
        #else:
        buf_str = arr[i:i+4]
        #print "horizontal=",buf_str
        num_sum = compute_list(buf_str)
        if num_sum > highest:   
            highest = num_sum
...
for i in xrange(0, 17):
    for x in xrange(0, 20):
        #if i+3 =< 21 and x+3 =< 21:
        #buf_str = [x[i] for x in overall[i:i+4]]
        buf_str = [overall[i][x],overall[i+1][x],\
                   overall[i+2][x],overall[i+3][x]]
        #print "vertical=",buf_str
        #else:
            #buf_str = [x[i] for x in overall[x:21]
        num_sum = compute_list(buf_str)
        if num_sum > highest:
            highest = num_sum


#this handles diagonal
for i in xrange(0, 17):#vertical
    for x in xrange(0, 17):
        buf_str = [overall[i][x], overall[i+1][x+1],\
                   overall[i+2][x+2], overall[i+3][x+3]]
        #print 'diag=',buf_str
        num_sum  = compute_list(buf_str)
        if num_sum > highest:
            highest = num_sum
    
    for x in xrange(19, 2, -1):
        buf_str =[overall[i][x], overall[i+1][x-1],\
                 overall[i+2][x-2], overall[i+3][x-3]]
        #print 'diag=',buf_str
        num_sum  = compute_list(buf_str)
